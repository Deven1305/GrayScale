...
```
  ...
  docs/forensics_report.md       │ → the FLOOR     │              │ 400 restored   │
  docs/DATASET.md                └────────┬────────┘              └────────────────┘
                                          │
                                          ▼
                                 ┌─────────────────┐
...
**The full no-ground-truth toolkit is in `docs/IMPROVEMENTS.md` Part 3.**
...
| `torch.cuda.is_available()` → False | Wrong PyTorch build — see `docs/SETUP.md` §2 |
